...
namespace TABS.Blazor.Menus;

public class TABSMenuContributor : IMenuContributor
{
    ...
    {
        var l = context.GetLocalizer<TABSResource>();
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                TABSMenus.HostDashboard,
                l["Menu:Dashboard"],
                "~/HostDashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(TABSPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                TABSMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "~/Dashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(TABSPermissions.Dashboard.Tenant)
        );



        context.Menu.AddItem(new ApplicationMenuItem(TABSMenus.Home, displayName: "测试用页面", "/ForTestPages", icon: "fa fa-warning")
     .AddItem(new ApplicationMenuItem(TABSMenus.Home, displayName: "测试用页面1", "/TestPage1"))
     .AddItem(new ApplicationMenuItem(TABSMenus.Home, displayName: "测试用页面2", "/TestPage2")));
